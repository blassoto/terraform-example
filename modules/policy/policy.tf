...
resource "aws_iam_role_policy" "beanstalk_policy" {
  name = "${var.app_name}-beanstalk-policy"
...
resource "aws_iam_instance_profile" "build" {
  name = "${var.app_name}-build-profile"
  role = aws_iam_role.build.name
}

resource "aws_s3_bucket_policy" "artifacts" {
  bucket = var.s3_bucket_artifacts_id
  policy =<<POLICY
{
  "Version": "2012-10-17",
  "Id": "${var.app_name}-artifacts-policy",
  "Statement": [
    {
      "Sid": "${var.app_name}-access",
      "Effect": "Allow",
      "Principal": {
        "AWS": "${aws_iam_role.build.arn}"
      },
      "Action": ["s3:ListBucket"],
      "Resource": ["${var.bucket_artifacts_arn}"]
    },
    {
      "Sid": "${var.app_name}-child-access",
      "Effect": "Allow",
      "Principal": {
        "AWS": "${aws_iam_role.build.arn}"
      },
